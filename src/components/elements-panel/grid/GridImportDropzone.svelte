<script lang="ts">
	...
	async function handleFiles(files: FileList | null) {
		...
		try {
			const result = await grid.handleImportFile(file, hasHeader);
...
>
	...
	<p class="text-muted-foreground mt-1 text-xs">You can also paste values directly into grid cells.</p>
...
<label class="mt-3 flex cursor-pointer items-center gap-2 text-xs">
	<input bind:checked={hasHeader} type="checkbox" class="accent-primary size-3 shrink-0 cursor-pointer" />
	<span>
		<span class="text-foreground">Use first row as column headers</span>
	</span>
</label>
