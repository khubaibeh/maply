...
| 10    | Release hardening and 50k acceptance run                  | 0-9        | In progress |
...
| 10    | PR #12 follow-up                   | —                                                           | [`acceptance.json`](benchmark-results/acceptance.json), [`indexed-soak.json`](benchmark-results/indexed-soak.json) | `pnpm test`, `pnpm build`, `pnpm check`, `pnpm check:deps`, `pnpm test:browser`, telemetry contract, 1,000-cycle 50k soak | In progress. The browser runner now retains individual frame samples and exposes the required 30-minute edit/undo/redo/save soak with heap and cache observations; the focused browser interaction test passes locally. Production-browser baseline/after reports are still required, so the acceptance artifact intentionally remains incomplete. |

## Final acceptance matrix

This matrix is intentionally incomplete until the required production-browser baseline/after
reports and 30-minute heap/cache soak artifacts are captured on the same documented environment.

| Capability            | 1k   | 10k  | 50k typical | 50k all visible | Evidence                                                   |
| --------------------- | ---- | ---- | ----------- | --------------- | ---------------------------------------------------------- |
| Load                  | pass | pass | pass        | pass            | Repository suites + fixture manifest                       |
| Pan/zoom              | pass | pass | pending     | pending         | Camera tests; 50k browser frame p95 pending                |
| Hover/pick            | pass | pass | pass        | pass            | Spatial query and interaction tests                        |
| Drag/resize           | pass | pass | pending     | pending         | Pointer/mutation tests; 50k browser frame p95 pending      |
...
| Memory/soak           | pass | pass | pending     | pending         | Node smoke soak; 30-minute browser heap/cache soak pending |

`pending` means functional suites and the deterministic engine/renderer gate may pass, but the
browser frame p95 and heap/cache measurements still require the documented browser lab. These
measurements are not silently substituted with a weaker editing path.
